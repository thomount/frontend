...
<script>
	import {mylogin, myregister} from '@/api/getData'
		...
		methods: {
			...
			async submitloginForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						const res = await mylogin({'username': this.loginForm.username, 'password': this.loginForm.password})
						console.log(res)
						if (res.status == 200) {
							this.$message({
		                        type: 'success',
			...
			async submitregisterForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						const res = await myregister({'username': this.loginForm.username, 'password': this.loginForm.password})
